...
def get_folders_between_timestamp(
    ...
    if not bucket_list:
        print("No folders in bucket list.")
        return []
    bucket_list_int = [int(bucket) for bucket in bucket_list]
    start_index = 0
    end_index = len(bucket_list_int) - 1
    while start_index < len(bucket_list_int) and int(
        bucket_list_int[start_index]
    ) < int(start_time):
        start_index += 1
    if start_index == len(bucket_list_int):
        print("No folders after start_time.")
        return []
    while end_index >= 0 and int(bucket_list_int[end_index]) > int(end_time):
        end_index -= 1
    if end_index < start_index:
        print("No folders in the specified range.")
        return []
    result = bucket_list[start_index : end_index + 1]
    print(result)
    print(f"First bucket in list: {bucket_list_int[start_index]}")
    print(f"Last bucket in list: {bucket_list_int[end_index]}")
    print(f"Start time as integer: {int(start_time)}")
    return result
...
def verify_playlist(bucket: str, folder_prefix: str, verbose: bool = False):
    ...
    referenced_ts = [
        ...
        if seg.uri and seg.uri.endswith(".ts")
    ]
    ...
    if verbose:
        if not missing_files and not extra_files:
            ...
            if missing_files:
                print(
                    f"[DISCREPANCY] {folder_prefix} missing files: {missing_files}"
                )
            if extra_files:
                print(
                    f"[DISCREPANCY] {folder_prefix} extra files: {extra_files}"
                )
