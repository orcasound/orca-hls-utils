...
class DateRangeHLSStream:
    ...
    def get_next_clip(self, current_clip_name=None):
        ...
        if num_total_segments == 0:
            ...
            return None, None, None
        # Only calculate target_duration if there are segments
        print('num_total_segments:', num_total_segments, 'length of valid folders:', len(self.valid_folders))
        target_duration = (
        ...
        # Failsafe: If segment_start_index is out of range, skip to next folder or end stream
        if segment_start_index >= num_total_segments:
            print("Segment start index exceeds total segments, current folder index:", self.current_folder_index, "length of valid folders:", len(self.valid_folders), "segment_start_index:", segment_start_index, "num_total_segments:", num_total_segments)
            if self.current_folder_index + 1 == len(self.valid_folders):
                print("End of stream reached, no more valid folders.")
                self.is_end_of_stream = True
                return None, None, None
            self.current_folder_index += 1
            self.current_clip_start_time = self.valid_folders[self.current_folder_index]
        ...
        if segment_end_index > num_total_segments:
            ...
            print("Segment end index exceeds total segments, current folder index:", self.current_folder_index, "segment_end_index:", segment_end_index, "num_total_segments:", num_total_segments)
