...
from orca_hls_utils import s3_utils, datetime_utils
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_midnight_boundary_crossing(self):
        ...
        with patch('orca_hls_utils.DateRangeHLSStream.s3_utils.get_all_folders') as mock_get_all_folders, \
             patch('orca_hls_utils.DateRangeHLSStream.s3_utils.get_folders_between_timestamp') as mock_get_folders_between, \
             patch('orca_hls_utils.DateRangeHLSStream.m3u8.load') as mock_m3u8:
            ...
            mock_playlist.segments = [MagicMock(duration=1.0, uri=f"live{i:03d}.ts") for i in range(10)]
            mock_m3u8.return_value = mock_playlist
            
            # This should not raise an exception
            stream = DateRangeHLSStream(
                self.stream_base, self.polling_interval, start_time, end_time, self.wav_dir
            )
    ...
    def test_very_short_time_window(self):
        ...
        with patch('orca_hls_utils.DateRangeHLSStream.s3_utils.get_all_folders') as mock_get_all_folders, \
            ...
            # Mock m3u8 with segments
            mock_playlist = MagicMock()
            mock_playlist.segments = [MagicMock(duration=1.0, uri=f"live{i:03d}.ts") for i in range(60)]
            mock_m3u8.return_value = mock_playlist
    ...
    def test_future_time_range(self):
        ...
        with patch('orca_hls_utils.DateRangeHLSStream.s3_utils.get_all_folders') as mock_get_all_folders, \
            ...
            mock_get_all_folders.return_value = []  # No folders exist for future time
    ...
    def test_empty_m3u8_playlist(self):
        ...
        with patch('orca_hls_utils.DateRangeHLSStream.s3_utils.get_all_folders') as mock_get_all_folders, \
            ...
            self.assertIsNone(clipname)
            self.assertTrue(stream.current_folder_index >= 1)  # Should have moved to next folder
    ...
    def test_segments_with_zero_duration(self):
        ...
        with patch('orca_hls_utils.DateRangeHLSStream.s3_utils.get_all_folders') as mock_get_all_folders, \
            ...
            durations = [item.duration for item in mock_playlist.segments if item.duration and item.duration > 0]
            if not durations:
                target_duration = 1.0  # Default used when all segments have zero duration
            else:
    ...
    def test_invalid_wav_directory(self):
        ...
        with patch('orca_hls_utils.DateRangeHLSStream.s3_utils.get_all_folders') as mock_get_all_folders, \
            ...
            with self.assertRaises((OSError, PermissionError)):
                DateRangeHLSStream(
                    self.stream_base, self.polling_interval, start_time, end_time, invalid_wav_dir
                )
    ...
    def test_disk_space_exhaustion_simulation(self):
        ...
        with patch('orca_hls_utils.DateRangeHLSStream.s3_utils.get_all_folders') as mock_get_all_folders, \
             patch('orca_hls_utils.DateRangeHLSStream.s3_utils.get_folders_between_timestamp') as mock_get_folders_between, \
             patch('orca_hls_utils.DateRangeHLSStream.m3u8.load') as mock_m3u8, \
             patch('orca_hls_utils.DateRangeHLSStream.ffmpeg.run') as mock_ffmpeg:
    ...
    def test_very_large_time_range(self):
        ...
        mock_folders = [str(start_time + i * 3600) for i in range(720)]  # 30 days * 24 hours
    ...
    def test_very_high_polling_interval(self):
        ...
        with patch('orca_hls_utils.DateRangeHLSStream.s3_utils.get_all_folders') as mock_get_all_folders, \
            ...
            # Mock m3u8 with segments
            mock_playlist = MagicMock()
            mock_playlist.segments = [MagicMock(duration=1.0, uri=f"live{i:03d}.ts") for i in range(600)]
            mock_m3u8.return_value = mock_playlist
            
            # Should handle high polling intervals
            stream = DateRangeHLSStream(
                self.stream_base, very_high_polling_interval, start_time, end_time, self.wav_dir
            )
            
            # Should not crash with high polling interval
            self.assertEqual(stream.polling_interval_in_seconds, very_high_polling_interval)
