...
class TestS3Utils(unittest.TestCase):
    ...
    def test_get_all_folders(self):
        ...
        for folder in test_folders:
            s3_client.put_object(
                Bucket=self.bucket_name, Key=folder + "live.m3u8", Body=b"test"
            )
    ...
    def test_get_folders_between_timestamp(self):
        ...
        # Test normal case
        result = s3_utils.get_folders_between_timestamp(
            bucket_list, start_time, end_time
        )
    ...
    def test_list_s3_files(self):
        ...
        for file_key in test_files:
            s3_client.put_object(
                Bucket=self.bucket_name, Key=file_key, Body=b"test"
            )
    ...
    def test_download_s3_object(self):
        ...
        with tempfile.NamedTemporaryFile() as tmp_file:
            s3_utils.download_s3_object(
                self.bucket_name, test_key, tmp_file.name
            )
            ...
            print(
                f"Successfully downloaded file with content: {downloaded_content}"
            )
    ...
    def test_verify_playlist_matching_files(
        self, mock_list_s3_files, mock_m3u8_loads, mock_s3fs
    ):
        ...
        # Test the function
        result = s3_utils.verify_playlist(
            self.bucket_name, "test-prefix/1700000000/"
        )
    ...
    @patch("orca_hls_utils.s3_utils.m3u8.loads")
    @patch("orca_hls_utils.s3_utils.list_s3_files")
    def test_verify_playlist_missing_files(
        self, mock_list_s3_files, mock_m3u8_loads, mock_s3fs
    ):
