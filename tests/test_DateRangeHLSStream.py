...
class TestDateRangeHLSStreamEdgeCases(unittest.TestCase):
    def setUp(self):
        ...
        self.unix_time_in_data = (
            1745564417  # Epoch time for '2025-04-28 07:00'
        )
        self.unix_time_out_data = int(
            dt.datetime(2025, 5, 5, 12, 0).timestamp()
        )  # Convert '2025-05-05T12:00' to epoch time
        self.unix_time_outer_data = int(
            dt.datetime(2025, 5, 6, 12, 0).timestamp()
        )  # Convert '2025-05-06T12:00' to epoch time
    ...
    def test_no_data_out_to_out(self):
        ...
        stream = DateRangeHLSStream(
            ...
            self.unix_time_out_data,
            self.unix_time_outer_data,
            self.wav_dir,
        )
